...
mT = np.array(
    ([1, 0, tx], [0, 1, ty], [0, 0, 1]), np.uint8
)  ##creamos nuestra matríz de transformación

imgT = np.zeros(
    (alto + 2, ancho + 10, 3), np.uint8
)  ##generamos una imagen base donde se guardará la imagen trasladada
...
for i in range(alto):  ##recorremos las filas de la matriz de la imagen
    for j in range(ancho):  ## recorremos las columnas de la matriz de la imagen
        ...
        dot = np.dot(
            mT, px
        )  ##aplicamos producto punto entre la matriz de transformación y el vector del pixel
...
mT = np.array(
    ([1, 0, tx], [0, 1, ty], [0, 0, 1]), np.uint8
)  ##generamos la matriz de traslación

imgT = np.zeros(
    (alto + ty, ancho + tx, 3), np.uint8
)  ##generamos la imagen en la cual se va a guardar la imagen trasladada


for i in range(alto):  ##recorremos las filas
    for j in range(ancho):  ##recorremos las columnas
        x = j + tx  ##sumamos el desplazamiento en x al índice de las columnas
        y = i + ty  ##sumamos el desplazamiento en y al índice de las filas
