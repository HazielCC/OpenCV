import cv2
import numpy as np


# Se obtiene la ruta del archivo actual
llave_1 = cv2.imread(
    "/level_2_introduction/part-1/pixels_llave.jpeg"
)  # Se lee la imagen a color
llave_2 = cv2.imread(
    "/level_2_introduction/part-1/pixels_llave_oxidada.jpeg"
)  # Se lee la imagen a color
alto, largo, canales = llave_1.shape  # Se obtienen las dimensiones de la imagen

# Se recorre la imagen
candado1 = llave_1[0:alto, 0 : int(largo / 2)]
candado2 = cv2.resize(llave_2[0:alto, int(largo / 2) :], (candado1.shape[1], alto))

# Unir las dos imágenes
imagen_unida = np.zeros((int(alto), int(largo), int(canales)), np.uint8)
imagen_unida[0:alto, 0 : int(largo / 2)] = candado1
imagen_unida[0:alto, int(largo / 2) : int(largo / 2) + candado2.shape[1]] = candado2

cv2.namedWindow("candado", cv2.WINDOW_NORMAL)
cv2.imshow("candado", imagen_unida)  # Se muestra la imagen
cv2.waitKey()  # Se espera a que se presione una tecla
